...
Base.getindex(g::GiftiStruct, intent::String) = data(g)[get(g.lookup, intent, Int[])]
...
has_pointset(g::GiftiStruct) = haskey(g.lookup, "NIFTI_INTENT_POINTSET")
has_triangle(g::GiftiStruct) = haskey(g.lookup, "NIFTI_INTENT_TRIANGLE")
has_pointsets(g::GiftiStruct) = has_pointest(g)
has_triangles(g::GiftiStruct) = has_triangle(g)
has_time_series(g::GiftiStruct) = haskey(g.lookup, "NIFTI_INTENT_TIME_SERIES")
